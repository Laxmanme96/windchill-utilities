...
public class EmptyRolesValidator implements RuleValidation {
	...
	public RuleValidationResult performValidation(RuleValidationKey paramRuleValidationKey,
		...
		logger.debug("EmptyRolesValidator result  : " + localRuleValidationStatus + " Message : " + localFeedbackMsg);
	...
	public void prepareForValidation(RuleValidationKey paramRuleValidationKey,
		...
		if (isCustomisationEnabled) {
			logger.debug("BEGIN validator : EmptyRoleRule. PBO is : "
		...
		if (!noParticipantList.isEmpty()) {
			...
			invalidObjectsMap.put(targetRefewrence,
					new RuleFeedbackMessage(
							new WTMessage("ext.emerson.windchill.promote.PromoteResource", "EMPTY_ROLE", localObject1),
	...
	public List<String> getEmptyRoles(PromotionNotice prObject) throws TeamException, WTException {
		...
		checkroles = props.getProperties("ext.emerson.windchill.promote.emptyRoleCheck");
		// if flag is Yes or No

		if (getCertificationReq(prObject)) {
			logger.debug("Cert reqd=true. Add roles : "
					+ props.getProperties("ext.emerson.windchill.promote.emptyRoleCheck.certReqd"));
			checkroles.addAll(props.getProperties("ext.emerson.windchill.promote.emptyRoleCheck.certReqd"));
		}
	...
	public Boolean getCertificationReq(PromotionNotice prNotice) throws WTException {

		Boolean cerflag = false;
		QueryResult qr = wt.maturity.MaturityHelper.service.getPromotionTargets(prNotice);
		while (qr.hasMoreElements()) {
			wt.fc.WTObject wto = (wt.fc.WTObject) qr.nextElement();

			String certAttr = props.getProperty("ext.emerson.windchill.promote.emptyRoleCheck.certReqd.attr");
			List<String> attrList = new ArrayList<String>();
			attrList.add(certAttr);
			HashMap<WTReference, HashMap<String, Object>> returnMap = CustomBusinessObjectHelper.fetchMultiObjIBA(wto,
					attrList);
			ReferenceFactory ref = new ReferenceFactory();
			WTReference wtref = ref.getReference(wto);
			logger.debug("wtref " + wtref + " All objects map : " + returnMap);
			HashMap<String, Object> ibaMap = returnMap.get(wtref);
			logger.debug("IBA map : " + ibaMap);
			if (ibaMap!= null && ibaMap.get(certAttr) != null && (boolean) (ibaMap.get(certAttr)) == true) {
				return true;
			}
