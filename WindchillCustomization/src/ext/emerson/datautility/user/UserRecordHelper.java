...
public class UserRecordHelper {
	...
	public static void main(String[] args) throws Exception {
		...
		UserRecordHelper helper = new UserRecordHelper("lam");
	 ...
	 * @throws Exception
	 */
	public UserRecordHelper(String searchTerm) throws Exception {
	...
	public List<UserRecord> searchDataContainedInAllColumns() throws Exception {
		...
		if (getSearchTerm() != null && getSearchTerm().length() > 0) {
			...
			statementBuilder.where().like(UserRecord.DISPLAY_NAME_FIELD, "%" + getSearchTerm() + "%").or()
					.like(UserRecord.ACCOUNT_NAME_FIELD, "%" + getSearchTerm() + "%").or()
					.like(UserRecord.ABBREVIATION_FIELD, "%" + getSearchTerm() + "%");
...
//		}
			...
			for (UserRecord rec : userRecords) {
				...
				String temp = rec.getAccountName() + "  " + rec.getAbreviation() + " " + rec.getDisplayName();
				String abbr = temp.split("\\s+")[1];
				logger.debug("abbreviation : " + abbr);
