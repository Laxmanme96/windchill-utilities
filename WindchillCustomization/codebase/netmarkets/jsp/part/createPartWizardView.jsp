...
<%--
FindSimilarObjectHelper.js below is required if classified part with showFindSimilarPart preference as true created
--%>
...
<c:choose>
   ...
   <c:when test='${param.showNewCADDocStep == "true"}'>
     ...
     <jca:wizard helpSelectorKey="${helpKey}" buttonList="${buttonList}" title="${wizardTitle}"
         ...
         <jca:wizardStep action="setClassificationAttributesWizStep" objectHandle="<%=PartConstants.ObjectHandles.PART%>" type="classification"/>
...
<script Language="JavaScript">
   ...
   function onMultipartsBack() {
    ...
    /*Start of customization for ES Classification Naming done on 05/13/25*/
    function setESClassificationNameFucntion() {

	  setTimeout(function() {
          console.log("inside setName function");
          
          var classificationNodeInternalName = "";
          var name = document.getElementById("ext.enersys.PART_LONG_DESCRIPTION");
          var classificationNode = document.querySelector('[id$="ENERSYS_CLASSIFICATION_BINDING_ATTR___textbox"]');

          var parent = classificationNode.parentElement;
			    var childNodesArr = parent.childNodes;
			    for (var i = 0; i < childNodesArr.length; i++) {
			      var node = childNodesArr[i];
			      if (node.type === "hidden") {
				      console.log("Hidden value:", node.value);
				      classificationNodeInternalName = node.value;
				      }
			      }

          var url = "netmarkets/jsp/ext/enersys/es/getClassificationDescription.jsp?classificationNode="
          + encodeURIComponent(classificationNodeInternalName);
          console.log(url);

          var xhr = new XMLHttpRequest();
          var descriptionPrompt = "";
          var example = "";
          var isAttributePresent = "";
          ...
          if(response){
            var description = response.split("#");
						descriptionPromptAndExample = description[0] ? description[0].trim() : "";
						isAttributePresent = description[1] ? description[1].trim() : "";
						
						var promptAndexample = descriptionPromptAndExample.split("+");
						descriptionPrompt = promptAndexample[0] ? promptAndexample[0].trim() : "";
						example = promptAndexample[1] ? promptAndexample[1].trim() : "";

            customToggleCreateCADDocStep('setClassificationAttributesWizStep!~objectHandle~partHandle~!', isAttributePresent);

            if (descriptionPrompt) {
              if (name.value) {
									showPopup(classificationNode.title,descriptionPrompt,example,name);
								}else{
									showPopupWithoutLongDescription(classificationNode.title,descriptionPrompt,example,name);
								}
            }
          }
              }
...
function customToggleCreateCADDocStep(wizardStep, isAttributePresent) {
    if (isAttributePresent === "true") {
        removeStep(wizardStep);
    }
}

if (typeof PTC !== "undefined" && PTC.driverAttributes) {
      console.log("PTC.driverAttributes is available");
      PTC.driverAttributes.on("afterRefresh", function () {
        console.log("afterRefresh triggered");
        setTimeout(() => {
          customToggleCreateCADDocStep('setClassificationAttributesWizStep!~objectHandle~partHandle~!');                         
        }, 2000); 
      });
    } else {
      console.warn("PTC.driverAttributes not available at DOMContentLoaded.");
    }

function showPopup(classificationNode, description, example, name){
	Ext.MessageBox.show({
	title:'Enersys Classification Information Window',
	msg: '<div style="word-wrap: break-word; font-family: Verdana, Arial, sans-serif; font-size: 13px;">' +
     '<h3 style="margin-bottom: 10px; color: #003865; font-weight: bold;">Selected Classification Node Info</h3>' +
     '<table style="border-collapse: collapse; width: 100%; border: 1px solid #bfbfbf;">' +
       '<tr>' +
         '<td style="padding: 6px 10px; font-weight: bold; background-color: #f0f0f0; border: 1px solid #bfbfbf;">Classification Node</td>' +
         '<td style="padding: 6px 10px; border: 1px solid #bfbfbf;">' + Ext.util.Format.htmlEncode(classificationNode) + '</td>' +
       '</tr>' +
       '<tr>' +
         '<td style="padding: 6px 10px; font-weight: bold; background-color: #f0f0f0; border: 1px solid #bfbfbf;">Description Prompt</td>' +
         '<td style="padding: 6px 10px; border: 1px solid #bfbfbf;">' + Ext.util.Format.htmlEncode(description) + '</td>' +
       '</tr>' +
       '<tr>' +
         '<td style="padding: 6px 10px; font-weight: bold; background-color: #f0f0f0; border: 1px solid #bfbfbf;">Example</td>' +
         '<td style="padding: 6px 10px; border: 1px solid #bfbfbf;">' + Ext.util.Format.htmlEncode(example) + '</td>' +
       '</tr>' +
     '</table>' +
	 '<br>' +
'<div style="margin-bottom: 10px; color: #000000; font-size: 10px;">Ok - Override Long description with above description prompt</div>' +
'<div style="margin-bottom: 10px; color: #000000; font-size: 10px;">Cancel - Keep Long Description As Is</div>' +
     '</div>',
...
function showPopupWithoutLongDescription(classificationNode, description, example, name){
	Ext.MessageBox.show({
	title:'Enersys Classification Information Window',
	msg: '<div style="word-wrap: break-word; font-family: Verdana, Arial, sans-serif; font-size: 13px;">' +
     '<h3 style="margin-bottom: 10px; color: #003865; font-weight: bold;">Selected Classification Node Info</h3>' +
     ...
     '</table>' +
	 '<br>' +
'<div style="margin-bottom: 10px; color: #000000; font-size: 10px;">Ok - Fill Long description with above description prompt</div>' +
'<div style="margin-bottom: 10px; color: #000000; font-size: 10px;">Cancel - Keep Long Description As Empty</div>' +
     '</div>',
	icon : Ext.MessageBox.INFO,
	modal: true,
	resizable: true,
...
/*End of customization for ES Classification Naming done on 05/13/25*/
